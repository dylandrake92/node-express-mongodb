...
beforeAll(async () => {
  ...
  // Ensure indexes exist in the test DB too.
...
describe("Tutorial API", () => {
  ...
  test("POST /api/tutorials returns 400 when title is too long", async () => {
    const longTitle = "A".repeat(101);

    const res = await request(app)
      .post("/api/tutorials")
      .send({ title: longTitle, description: "too long title" })
      .expect(400);
  ...
  test("GET /api/tutorials/:id returns 400 for invalid id format", async () => {
    const res = await request(app).get("/api/tutorials/not-a-real-id").expect(400);
    expect(res.body).toHaveProperty("error");
    expect(res.body.error).toHaveProperty("message");
  });

  test("DB has expected indexes", async () => {
    const indexes = await db.tutorials.collection.indexes();
    const names = indexes.map(i => i.name);

    expect(names).toEqual(
      expect.arrayContaining(["title_1", "published_1_createdAt_-1"])
    );
  });
